...
bodies_to_create = ["Europa","Jupiter"]
...
"Jupiter", global_frame_orientation, "Europa_Fixed")

body_system = environment_setup.create_system_of_bodies(body_settings)

# Librations
scaled_libration_amplitude = 10.0
libration_calculator = environment.DirectLongitudeLibrationCalculator(scaled_libration_amplitude)
body_system.get("Europa").rotation_model.libration_calculator = libration_calculator
...
time_lag_eur = 1.0724e3 # 7827.4
time_lag_jup = 0.09
# Add entry to acceleration settings dict
acceleration_settings_eur = dict(
...
acceleration_settings = {"Europa": acceleration_settings_eur}
...
dependent_variables_to_save = [
    ...
    propagation_setup.dependent_variable.latitude("Europa","Jupiter"),
    propagation_setup.dependent_variable.latitude("Europa","Jupiter")
]
...
def plot_multi_body_system_state_history(states_array, bodies_to_propagate):
    ...
    ax1.set_title(f'Trajectory of Europa w.r.t Jupiter')
...
#dedt = (57*0.379*4.799844e22*2*np.pi/310000.0)*eccentricity[0]*((69911000/semi_major_axis[0])**5)/(35600*8*1.8982e27) # Planet
...
fig.suptitle('Kepler elements variation of Europa due to tides')
...
fig2.suptitle('Latitude and longitude of Europa during the propagation')
