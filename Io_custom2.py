...
initial_keplerian_state=element_conversion.cartesian_to_keplerian(initial_cartesian_state, jupiter_gravitational_parameter)

a0=initial_keplerian_state[0]
theta0=initial_keplerian_state[5]
e0=initial_keplerian_state[1]
n0=np.sqrt(jupiter_gravitational_parameter/(a0**3))


#Mean anomaly
arg1=np.sqrt(1-e0**2)*np.sin(theta0)/(1+e0*np.cos(theta0))
arg2=(e0+np.cos(theta0))/(1+e0*np.cos(theta0))
E=np.arctan2(arg1,arg2)
M=E-e0*np.sin(E)
psi=-2*e0*np.sin(M)+np.pi

# Spherical harmonics variation in time
gravity_field_variation_settings = list()
tide_raising_body = "Jupiter" #"Io"
degree = 2
...
RSW_matrix = frame_conversion.inertial_to_rsw_rotation_matrix(initial_cartesian_state)
rotation_matrix=[
    [np.cos(psi),       np.sin(psi),   0],
    [-np.sin(psi),      np.cos(psi),   0],
    [0,                 0,             1]]
initial_orientation=np.dot(RSW_matrix,rotation_matrix)
...
n2=np.sqrt(jupiter_gravitational_parameter/(semi_major_axis[len(semi_major_axis)-1]**3))
ndot=(n2-n0)/(time[len(time)-1]-time[0])
n_Io=list()
j=0.0
while j<len(time):
    n=np.sqrt(jupiter_gravitational_parameter/(semi_major_axis[j]**3))
    n_Io.append(n)
    j=j+1

plt.figure(figsize=(10,6))
plt.title("Mean motion variation over time")
plt.plot(time_day, n_Io, 'r')
